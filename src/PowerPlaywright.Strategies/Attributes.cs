namespace PowerPlaywright.Strategies
{
    /// <summary>
    /// Defines constants for common HTML attributes used in Playwright strategies.
    /// </summary>
    internal class Attributes
    {
        /// <summary>
        /// Gets the name of the 'class' attribute.
        /// </summary>
        internal const string Class = "class";

        /// <summary>
        /// Gets the name of the 'id' attribute.
        /// </summary>
        internal const string Id = "id";

        /// <summary>
        /// Gets the name of the 'aria-colcount' attribute.
        /// </summary>
        internal const string AriaColCount = "aria-colcount";

        /// <summary>
        /// Gets the name of 'aria-controls' attribute.
        /// </summary>
        internal const string AriaControls = "aria-controls";

        /// <summary>
        /// Gets the name of 'aria-expanded' attribute.
        /// </summary>
        internal const string AriaExpanded = "aria-expanded";

        /// <summary>
        /// Gets the name of 'aria-label' attribute.
        /// </summary>
        internal const string AriaLabel = "aria-label";

        /// <summary>
        /// Gets the name of 'data-fieldrequirement' attribute.
        /// </summary>
        internal const string DataFieldRequirement = "data-fieldrequirement";

        /// <summary>
        /// Gets the name of 'data-lp-id' attribute.
        /// </summary>
        internal const string DataLpId = "data-lp-id";
    }
}
